...
<script setup>
import { Terminal as TerminalIcon, Download, Mail, Github, HelpCircle, Gamepad2, Ghost, BookOpen, Palette } from 'lucide-vue-next'
...
const commands = [
  ...
  { value: 'designs --all', label: 'designs --all', description: 'Open the web graphic designs gallery', icon: Palette },
...
function executeCmd(cmd) {
  ...
  if (cmd === 'designs --all') {
    output.value.push({ type: 'output', text: 'Opening /designs ...' })
    input.value = ''
    setTimeout(() => {
      emit('update:open', false)
      router.push('/designs')
